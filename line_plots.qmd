...
for (i in deep_11) {
  ...
  annotate('text', x = 9.75, y = 55, label = 'Enceladus ocean water') +
  theme_classic() +
    ggtitle(paste0(names(deep_11)[i], 'pH = 11')) 
  ggsave(path = '/figs', device = 'png', filename = paste0(names(deep_11)[i], 'ph_11'), plot = p)
  }
```

```{r}
alanine <- as.data.frame(matrix(unlist(deep_11$ala), ncol = 5, nrow = 21))
